...
Usuário vs Contas: Um Usuário pode possuir várias Contas (1:N), mas uma conta pertence a um único usuário.

Conta vs Transações: Uma Conta realiza diversas Transações (1:N).

Transações vs Categorias: Cada Transação pertence a uma Categoria específica (N:1).

Conta vs Serviços: Uma Conta é a origem de múltiplas operações de PIX, Pagamentos, Recargas e recebimento de Cashback (1:N).
...
3. Execute o Script:
Abra o arquivo script_completo.sql e execute (tecla F5). O script irá:
